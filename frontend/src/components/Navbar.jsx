...
export default function NavBar({ variant = "app" }) {
    ...
    return (
        <header className="flex h-auto items-center justify-between gap-4 border-b border-slate-200 bg-white px-4 py-4 sm:h-[72px] sm:px-7 sm:py-0">
            <NavLink className="flex items-center gap-3 text-xl font-bold text-slate-950" to="/user">
                <span className="grid h-8 w-8 place-items-center rounded-lg bg-indigo-600 text-white">C</span>
                <span>CODO</span>
            </NavLink>

            <div className="flex items-center gap-3">
                <NavLink className="flex items-center gap-3 text-slate-950" to="/user/profile">
                    <span className="hidden text-right sm:grid">
                        <strong>Aditya Sharma</strong>
                    </span>
                    <span className="grid h-10 w-10 place-items-center rounded-full bg-slate-950 text-sm font-bold text-white">AS</span>
                </NavLink>
            </div>
        </header>
    )
