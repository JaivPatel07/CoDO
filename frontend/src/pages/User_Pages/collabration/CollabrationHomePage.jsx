  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm p-6 animate-pulse">
      ...
      <div className="space-y-2 mb-5">
        <div className="h-6 bg-slate-100 dark:bg-slate-800 rounded w-4/5" />
      ...
      <div className="flex items-center justify-between">
        <div className="h-9 bg-slate-100 dark:bg-slate-800 rounded-lg w-24" />
        <div className="h-9 bg-slate-100 dark:bg-slate-800 rounded-lg w-28" />
      </div>
    </div>
  );
}
...
export default function CollaborationHomePage() {
  ...
  return (
    <div className="animate-in fade-in duration-500 bg-slate-50 dark:bg-slate-800/50 min-h-screen">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            ...
            <div className="relative w-full lg:w-72 shrink-0">
              ...
              <input
                ...
                className="w-full h-11 pl-10 pr-4 bg-slate-50 dark:bg-slate-800 border border-slate-200 rounded-xl text-sm font-medium text-slate-900 dark:text-slate-100 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 transition-all"
              />
            </div>
                ...
                {tabs.map((tab) => (
                  <button
                    ...
                    className={`whitespace-nowrap text-sm font-bold px-4 py-2.5 rounded-xl transition-all duration-200 focus:outline-none ${filter === tab
                      ...
                      : 'bg-white border border-slate-200 text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:bg-slate-800 hover:text-slate-800'
                      }`}
                  >
                    {tab}
                  </button>
            ...
            <div className="relative shrink-0" ref={dropdownRef}>
              <button
                ...
                className="flex items-center justify-between w-full lg:w-44 h-11 px-4 bg-white border border-slate-200 rounded-xl text-sm font-bold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:bg-slate-800 transition focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500"
              >
                  ...
                  {sortOptions.map((option) => (
                    <button
                      ...
                      className={`w-full text-left px-3 py-2.5 text-sm rounded-lg transition-colors ${sort === option.value
                        ? 'bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-slate-100 font-bold'
                        : 'text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:bg-slate-800 hover:text-slate-800 font-semibold'
                        }`}
                    >
                      {option.label}
                    </button>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-10">
        ...
        {isLoading ? (
          ...
          <div className="bg-red-50/50 border border-red-100 rounded-3xl p-8 text-center flex flex-col items-center justify-center min-h-[300px]">
            ...
            <button onClick={() => window.location.reload()} className="mt-6 px-6 py-2.5 bg-white border border-slate-200 rounded-xl font-bold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:bg-slate-800 transition-colors shadow-sm">
              Try Again
            </button>
          </div>
          ...
          <div className="bg-white border border-slate-200 rounded-3xl p-12 text-center flex flex-col items-center justify-center min-h-[400px] shadow-sm">
            <div className="w-20 h-20 bg-slate-50 dark:bg-slate-800 text-slate-300 rounded-[24px] flex items-center justify-center mb-6 ring-8 ring-slate-50/50">
              <LayoutGrid size={40} />
            </div>
            <h2 className="text-2xl font-black text-slate-900 dark:text-slate-100 mb-3">No Opportunities Found</h2>
            <p className="text-slate-500 dark:text-slate-400 max-w-md mb-8 text-lg">We couldn't find any posts matching your criteria. Try adjusting your filters or search terms.</p>
            <Link to='../createpost'>
