...
import {
  ...
  ShieldCheck, Circle, Clock, Activity, Terminal,
  Trash2, X // <-- Added Trash2 and X icons here
} from 'lucide-react';
...
import { get_networks, remove_network } from '../../../api/networks_api';
...
const ProfileHero = ({ profile, user, isOwnProfile, handle_editprofile, handleCopy, copied }) => (
        ...
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
          ...
          >
            {isOwnProfile ? (
              ...
              <>
                <button className="flex-1 md:flex-none flex justify-center items-center gap-2 bg-zinc-900 hover:bg-zinc-800 text-white px-5 py-2.5 rounded-xl text-sm font-medium transition-all active:scale-95 shadow-sm">
...
// Updated to accept proper objects with label, value, and onClick
...
// --- NEW MODAL COMPONENT FOR CONNECTIONS ---
const ConnectionsModal = ({ isOpen, onClose, connections }) => {
  if (!isOpen) return null;

  const handleDeleteNetwork = async(name,user_id) => {
    try {
      await remove_network(user_id)
      alert(`${name} remove from networks!!!`)
    }
    catch (err) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
      <div 
        ...
        onClick={(e) => e.stopPropagation()} // prevent click from closing if clicked inside
      >
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-zinc-100">
          <h3 className="text-lg font-bold text-zinc-900 flex items-center gap-2">
            <UserPlus size={18} className="text-zinc-500" /> My Connections
          </h3>
          <button 
            onClick={onClose} 
          ...
          {connections.length === 0 ? (
            ...
            connections.map((conn, idx) => (
              ...
              >
                ...
                <button onClick={()=> handleDeleteNetwork(conn.fullname,conn.user_id)}
...
const ProfilePage = () => {
  ...
  useEffect(() => {
    ...
    const fetchConnection = async () => {
      try {
        const data = await get_networks();
        setMyNetworks(data.filter((d)=> d.status === "accepted"));
      }
      catch (err) {
        console.log(err?.response);
      }
  ...
  return (
    <div className="min-h-screen bg-zinc-50 font-sans text-zinc-900 pb-20 selection:bg-blue-100 selection:text-blue-900">
            ...
            {tabs.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`relative flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-bold transition-all whitespace-nowrap ${
