      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-slate-500 dark:text-slate-400 mt-1">Manage your workspaces and team collaborations.</p>
          </div>
          <button 
        ...
        {/* Filters */}
        <div className="flex items-center gap-2 mb-8 bg-white dark:bg-slate-900 p-1.5 rounded-xl border border-slate-200 dark:border-slate-700 inline-flex shadow-sm">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl overflow-hidden shadow-sm">
          {/* Header */}
          <div className="grid grid-cols-12 px-6 py-4 bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700 text-sm font-semibold text-slate-600 dark:text-slate-400">
          ...
          {filteredTeams.map((team) => (
            <div
              key={team.id}
              className="grid grid-cols-12 items-center px-6 py-5 border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800 transition"
            >
              {/* Team Name */}
                ...
                {team.role === "lead" && (
                  <>
                    <button
                      ...
                      className={`p-2 rounded-lg transition ${
                        copiedTeamId === team.id 
                          ...
                          : 'hover:bg-indigo-50 text-slate-500 dark:text-slate-400 hover:text-indigo-600'
                      }`}
                    ...
                    <button
                      ...
                      className="p-2 rounded-lg hover:bg-red-50 text-slate-500 dark:text-slate-400 hover:text-red-600 transition"
                      title="Delete Team"
                    >
                      <Trash2 size={18} />
                    </button>
          ...
          {filteredTeams.length === 0 && (
            <div className="py-16 text-center">
              <Users size={50} className="mx-auto text-slate-300 dark:text-slate-500 mb-3" />
         ...
         <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 dark:bg-slate-950/80 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-700">
              ...
              <button 
                onClick={closeModal}
                className="p-2 text-slate-400 dark:text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-600 dark:hover:text-slate-300 rounded-full transition"
              >
                <X size={20} />
              </button>
            </div>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Frontend Developers"
                   className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white dark:focus:bg-slate-800 transition"
                  autoFocus
                />
              </div>
            </div>
            ...
            {/* Modal Footer */}
            <div className="flex items-center justify-end gap-3 px-6 py-4 bg-slate-50 dark:bg-slate-900 border-t border-slate-100 dark:border-slate-700">
              <button
                onClick={closeModal}
