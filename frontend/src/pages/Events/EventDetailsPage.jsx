    ...
    return (
        <div className="min-h-screen bg-slate-50/50 pb-8">
            <main className="px-4 md:px-6 max-w-7xl mx-auto animate-in fade-in duration-300">
                ...
                {/* Hero Section */}
                <section className="relative w-full h-[220px] md:h-[280px] rounded-2xl overflow-hidden mb-5 shadow-md shadow-slate-100 border border-slate-100">
                    {event.banner_image ? (
                        <>
                ...
                <div className="grid lg:grid-cols-12 gap-5">
                    ...
                    <div className="lg:col-span-8 space-y-5">
                        ...
                        {isOwner ? (
                            <div className="flex items-center justify-between gap-4 bg-white px-5 py-3 rounded-2xl shadow-sm border border-slate-100">
                                <div className="min-w-0">
                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Managing Event</p>
                                    <p className="text-sm font-bold text-slate-700 mt-0.5 truncate">{event.title}</p>
                                </div>
                                <div className="flex items-center gap-2 shrink-0">
                                    <button
                                        onClick={() => navigate(`/organization/${userData.username}/events/edit/${event.id}`)}
                                        title="Edit Event"
                                        className="w-8 h-8 flex items-center justify-center rounded-xl bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-600 hover:text-slate-900 transition-all cursor-pointer shadow-sm"
                                    >
                                        <Edit size={14} />
                                    </button>
                                    <button
                                        ...
                                        title="Delete Event"
                                        className="w-8 h-8 flex items-center justify-center rounded-xl bg-red-50 hover:bg-red-100 border border-red-100 text-red-500 hover:text-red-700 transition-all cursor-pointer shadow-sm disabled:opacity-50"
                                    >
                                        {deleting
                            ...
                            <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100/80 flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition-all hover:shadow-md">
                                <div className="min-w-0 flex-1">
                                    <h2 className="text-lg sm:text-xl font-black text-slate-900 tracking-tight">Secure Your Spot</h2>
                        ...
                        <div className="grid sm:grid-cols-2 gap-4">
                            ...
                            <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-200/60 flex items-start gap-3 transition-all duration-300 hover:shadow-md hover:border-slate-300/80 relative group">
                                ...
                                <div className="min-w-0 flex-1">
                                    <h4 className="font-bold text-[9px] sm:text-[10px] text-slate-400 uppercase tracking-widest mb-1 font-sans">Date & Timing</h4>
                                    <p className="font-bold text-sm text-slate-800 tracking-tight leading-snug">
                            ...
                            <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-200/60 flex items-start gap-3 transition-all duration-300 hover:shadow-md hover:border-slate-300/80 relative group">
                                ...
                                <div className="min-w-0 flex-1">
                                    <h4 className="font-bold text-[9px] sm:text-[10px] text-slate-400 uppercase tracking-widest mb-1 font-sans">Location</h4>
                                    <p className="font-bold text-sm text-slate-800 tracking-tight leading-snug truncate">
                        ...
                        {/* Event Details */}
                        <div className="bg-white p-5 sm:p-6 rounded-2xl shadow-sm border border-slate-200/60 transition-all duration-300 hover:shadow-md">
                            <h3 className="text-lg font-black text-slate-900 mb-4 tracking-tight border-b border-slate-100 pb-2.5">About the Event</h3>
                        ...
                        {event.custom_dates && typeof event.custom_dates === 'object' && Object.keys(event.custom_dates).length > 0 && (
                            <div className="bg-white p-6 sm:p-8 rounded-3xl shadow-sm border border-slate-200/60 transition-all duration-300 hover:shadow-md">
                                <div className="flex items-center gap-3 mb-8 border-b border-slate-100 pb-4">
                                    ...
                                    <div>
                                        <h3 className="text-lg sm:text-xl font-black text-slate-900 tracking-tight">Event Schedule</h3>
                                    ...
                                    {Object.entries(event.custom_dates).map(([label, date], index, arr) => {
                                        ...
                                        return (
                                            <div key={label} className="relative flex gap-6 items-start group">
                                                ...
                                                <div className="relative z-10 w-12 h-14 rounded-2xl border-2 border-slate-200 bg-white overflow-hidden shadow-sm flex flex-col items-center justify-center shrink-0 group-hover:border-violet-500 group-hover:shadow-md group-hover:shadow-violet-100 transition-all duration-300">
                                                    <div className="bg-slate-50 group-hover:bg-violet-600 text-slate-400 group-hover:text-white text-[9px] font-black w-full text-center py-1 tracking-wider uppercase transition-colors duration-300 border-b border-slate-100 group-hover:border-violet-700">
                                                        {monthStr}
                                                    </div>
                                                    <div className="text-base font-black text-slate-800 group-hover:text-violet-650 w-full text-center flex-1 flex items-center justify-center font-mono leading-none">
                                                        {dayStr}
                                                    </div>
                                                </div>

                                                {/* Right Column: Content Card */}
                                                <div className="min-w-0 flex-1 bg-slate-50/50 hover:bg-white rounded-2xl px-4 py-3.5 transition-all duration-300 border border-slate-100 group-hover:border-violet-100 group-hover:shadow-sm flex items-center">
                                                    <h4 className="text-sm font-extrabold text-slate-850 tracking-tight leading-tight group-hover:text-slate-900 transition-colors">
                                                        {label}
                                                    </h4>
                                                </div>
                                            </div>
                        ...
                        {event.tags && (
                            <div className="flex flex-wrap gap-2 pt-2">
                                {event.tags.split(",").map((tag) => (
                                    <span key={tag} className="px-3.5 py-1.5 bg-slate-100 text-slate-650 hover:text-slate-900 border border-slate-200/50 rounded-full font-bold text-xs transition-all duration-205 cursor-default hover:bg-slate-200/60">
                                        #{tag.trim()}
                                    </span>
                                ))}
                            </div>
                    ...
                    <aside className="lg:col-span-4 space-y-6">
                        <div className="space-y-6">
                            ...
                            {(() => {
                                if (isOnline) {
                                    return (
                                        <div className="bg-white p-4 sm:p-5 rounded-2xl shadow-sm border border-slate-100 overflow-hidden relative transition-all hover:shadow-md">
                                            ...
                                            <div className="bg-gradient-to-br from-violet-50 to-indigo-50/50 p-4 rounded-xl border border-violet-100 flex flex-col items-center text-center">
                                                ...
                                                <h4 className="font-bold text-slate-800 text-xs mb-0.5">This Event is Virtual</h4>
                                            ...
                                            {event.registration_link && (
                                                <div className="mt-3 pt-3 border-t border-slate-100">
                                                    <a
                                                        href={event.registration_link}
                                                        onClick={handleRegistrationClick}
                                                        className="w-full h-9 bg-violet-600 hover:bg-violet-700 text-white font-bold text-xs rounded-xl active:scale-95 transition-all flex items-center justify-center gap-1.5 shadow-sm shadow-violet-600/10"
                                                    >
                                ...
                                if (event.map_link || event.location) {
                                    return (
                                        <div className="bg-white p-4 sm:p-5 rounded-2xl shadow-sm border border-slate-100 overflow-hidden relative group/map transition-all hover:shadow-md">
                                            <div className="flex justify-between items-center gap-2 mb-3">
                                                <div className="flex items-center gap-1.5 min-w-0">
                                                    <Compass className="text-violet-600 animate-[spin_8s_linear_infinite] shrink-0" size={14} />
                                                    <h3 className="text-xs font-bold text-slate-800 tracking-wide">Location</h3>
                                                </div>
                                                <button
                                            ...
                                            <div className="pt-2">
                                                ...
                                                <div className="grid grid-cols-2 gap-2 mt-4">
                                                    ...
                                                    <a
                                                        ...
                                                        className="h-10 bg-violet-600 hover:bg-violet-700 text-white font-semibold text-xs rounded-xl active:scale-95 transition-all flex items-center justify-center gap-1.5 shadow-sm shadow-violet-600/10"
                                                    >
                                                        <Navigation size={14} />
                ...
                {isMapExpanded && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
                        <div className="bg-white w-full max-w-4xl rounded-3xl overflow-hidden shadow-2xl border border-slate-100 flex flex-col h-[80vh] sm:h-[85vh] animate-in zoom-in-95 duration-200">
                            {/* Modal Header */}
                            <div className="p-4 sm:p-5 border-b border-slate-100 flex justify-between items-center shrink-0 bg-slate-50">
                                ...
                                <button
                                    ...
                                    className="h-9 w-9 bg-white hover:bg-slate-100 border border-slate-200 text-slate-500 hover:text-slate-800 rounded-full flex items-center justify-center transition-all shadow-sm cursor-pointer shrink-0"
                                >
                                    <X size={18} />
                                </button>
                            </div>
                            ...
                            <div className="p-4 border-t border-slate-100 bg-white flex flex-col sm:flex-row justify-between items-center gap-3 shrink-0">
                                <span className="text-[11px] sm:text-xs text-slate-400 font-medium hidden sm:inline">Interactive navigation powered by Google Maps</span>
                                <div className="flex gap-2 w-full sm:w-auto">
                                    ...
                                    <a
                                        ...
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className="flex-1 sm:flex-initial h-9 px-5 bg-violet-600 text-white font-semibold text-xs rounded-xl hover:bg-violet-700 active:scale-95 transition-all flex items-center justify-center gap-1.5 shadow-sm cursor-pointer"
                                    >
                                        <Navigation size={13} />
