...
import { ChevronLeft, ChevronRight, MapPin, ArrowRight, Calendar as CalendarIcon, Clock, Users, Search, Bookmark, Building2, LayoutGrid, Zap } from "lucide-react";
...
const FILTERS = ["All", "Hackathons", "Workshops", "Competitions", "Meetups", "Webinars", "Tech", "Design", "Business", "Sports"];
...
export default function CalendarPage() {
    ...
    // Filter events basically (search query only for now, logic is minimal)
    const displayEvents = events.filter(e => 
        (e.title?.toLowerCase().includes(searchQuery.toLowerCase()) || 
        e.organization_name?.toLowerCase().includes(searchQuery.toLowerCase()))
    );

    const eventsOnDay = (y, m, d) => displayEvents.filter(e => e.event_date === formatKey(y, m, d));
    ...
    const groupedEvents = useMemo(() => {
        const groups = { "Today": [], "Tomorrow": [], "This Week": [], "This Month": [], "Later": [], "Past": [] };
        
        // Base date for grouping (usually today, but if a date is clicked, we filter to just that day)
        if (clickedDate) {
            const key = formatKey(clickedDate.getFullYear(), clickedDate.getMonth(), clickedDate.getDate());
            groups[clickedDate.toDateString() === today.toDateString() ? "Today" : clickedDate.toLocaleDateString('en-US', { weekday: 'long', month: 'short', day: 'numeric' })] = displayEvents.filter(e => e.event_date === key);
            return groups;
        }
        ...
        Object.keys(groups).forEach(k => {
            groups[k].sort((a,b) => new Date(a.event_date) - new Date(b.event_date));
        });
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12 animate-in fade-in duration-500">
                    <div className="max-w-3xl mb-8">
                        ...
                        <p className="text-lg text-slate-500 font-medium">
                            Discover hackathons, workshops, competitions, webinars, and networking events organized by communities and organizations.
                        </p>
                    </div>
